...
Config::Config() :
	...
	m_sVirtualCameraParameterFile(""),
	m_uiWidth(0),
	m_uiHeight(0),
	m_uiPrioritization(0),
	m_uiDepthBasedPrioritization(0),
	m_uiFormat(0),
	m_dAovL(-PI),
	m_dAovR(PI),
	m_dAovT(PI_HALF),
...
void Config::readCfgFile(const char* filename) {
	...
	while (!feof(fileCfg)) {
		...
		if (tmpPar[0] == '#') fgets(tmpPar, MAX_TEXT_LENGTH, fileCfg);
			...
			if (!strcmp(tmpPar, "VirtualCameraParameterFile")) m_sVirtualCameraParameterFile = tmpVal;

			if (!strcmp(tmpPar, "Width")) m_uiWidth = atoi(tmpVal);
			if (!strcmp(tmpPar, "Height")) m_uiHeight = atoi(tmpVal);
			...
			if (!strcmp(tmpPar, "RemoveGhostEdges")) m_uiRemoveGhostEdges = atoi(tmpVal);

			if (!strcmp(tmpPar, "Prioritization")) m_uiPrioritization = atoi(tmpVal);
			if (!strcmp(tmpPar, "DepthBasedPrioritization")) m_uiDepthBasedPrioritization = atoi(tmpVal);
			...
			if (!strncmp(tmpPar, "Input", 5) || !strncmp(tmpPar, "Output", 6)) {
				while (1) {
					fscanf(fileCfg, "%s", tmpPar, MAX_TEXT_LENGTH);
	...
	m_cfgOutputs = new CfgYUVParams[m_uiNumberOfOutputViews + 1]; //+1, ostatni to default
	...
	for (Int i = 0;i < m_uiNumberOfInputViews; i++) {
		...
		m_cfgInputs[i].m_uiWidth = m_uiWidth;
		m_cfgInputs[i].m_uiHeight = m_uiHeight;
	...
	for (Int o = 0;o < m_uiNumberOfOutputViews + 1; o++) {
		...
		m_cfgOutputs[o].m_uiWidth = m_uiWidth;
		m_cfgOutputs[o].m_uiHeight = m_uiHeight;
			...
			if (!strncmp(tmpPar, "Input", 5) && inputCnt < m_uiNumberOfInputViews) {
				while (1) {
					...
					if (tmpPar[0] == '#' || tmpPar[0] == '{') fgets(tmpPar, MAX_TEXT_LENGTH, fileCfg);
						...
						if (!strcmp(tmpPar, "Width")) m_cfgInputs[inputCnt].m_uiWidth = atoi(tmpVal);
						if (!strcmp(tmpPar, "Height")) m_cfgInputs[inputCnt].m_uiHeight = atoi(tmpVal);
						...
						if (!strcmp(tmpPar, "Priority")) m_cfgInputs[inputCnt].m_uiPriority = atoi(tmpVal);
...
#else
						if (!strcmp(tmpPar, "CameraName")) m_cfgOutputs[outputCnt].m_sCameraName = tmpVal;
#endif
						if (!strcmp(tmpPar, "View")) m_cfgOutputs[outputCnt].m_sViewFilename = tmpVal;
						if (!strcmp(tmpPar, "Depth")) {
							m_cfgOutputs[outputCnt].m_sDepthFilename = tmpVal;
							m_cfgOutputs[outputCnt].m_bDepthFileWritingEnabled = true;
						}

						if (!strcmp(tmpPar, "Width")) m_cfgOutputs[outputCnt].m_uiWidth = atoi(tmpVal);
						if (!strcmp(tmpPar, "Height")) m_cfgOutputs[outputCnt].m_uiHeight = atoi(tmpVal);
	...
	fclose(fileCamParam);
	fileCamParam = fopen(m_sVirtualCameraParameterFile.c_str(), "r");

	if (fileCamParam == NULL) {
		fprintf(stdout, "Error: parameter file %s does not exist\n", m_sVirtualCameraParameterFile.c_str());
		return;
	}

	for (Int o = 0;o < m_uiNumberOfOutputViews;o++) {
		for (Int oo = 0;oo < m_cfgOutputs[o].m_asCameraNames.size();oo++) {
			...
			while (!feof(fileCamParam)) {
				...
				if (!strcmp(tmpPar, m_cfgOutputs[o].m_asCameraNames[oo].c_str())) {
					...
					//m_cfgOutputs[o].m_camParams = new CamParams();
